...
IMPORTANT_KEYWORDS = {
    ...
    "unhealthy",
    # HTTP error codes
    # Note: 400/401/403/404 are intentionally excluded. They are almost
    # entirely normal frontend auth-refresh flow (401) or internet bot
    # scanning noise (404) and carry no diagnostic value. Only codes that
    # reliably indicate a real server-side problem are kept.
    "408",
    "409",
    "500",
    "502",
...
# Note: LoggerLabel values are intentionally NOT included here anymore.
# They used to be injected as standalone keywords, which meant any
# routine, non-error log line tagged with a service label (e.g. the
# normal startup message "[TeleinfoListener] Listening for data...")
# was flagged as important just because it carried that tag.

# Important: use lowercase
NOT_IMPORTANT_KEYWORDS = {
    ".service: consumed",
    "available power is too low",
    # Routine gunicorn worker lifecycle noise (happens on every restart/
    # deploy, no diagnostic value on its own). Service-level restart
    # events (e.g. "Stopped X.service", "Scheduled restart job, restart
    # counter is at N") are intentionally NOT filtered: they're the
    # signal that revealed the teleinfo-listener flapping incident.
    "booting worker",
    "worker exiting",
    "using worker:",
}
