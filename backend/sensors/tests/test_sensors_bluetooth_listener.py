...
class TestDetectionCallback:
    ...
    def test_valid_measurement_with_temperature_is_buffered(self):
        ...
        with (
            patch(
                "sensors.bluetooth_listener.decode_bthome_payload",
                return_value={"battery": 60, "temperature": 20.5},
            ),
            patch("sensors.bluetooth_listener.timezone.now") as mock_now,
        ):
    ...
    def test_undecodable_payload_is_ignored(self):
        ...
        with patch(
            "sensors.bluetooth_listener.decode_bthome_payload", return_value=None
        ):
            self.listener.detection_callback(device, advertisement_data)
    ...
    def test_zero_temperature_is_still_buffered(self):
        ...
        with (
            ...
            patch(
                "sensors.bluetooth_listener.timezone.now",
            ) as mock_now,
        ):
            mock_now.return_value.isoformat.return_value = "2026-01-01T00:00:00"
            self.listener.detection_callback(device, advertisement_data)
    ...
    def test_device_without_name_falls_back_to_unknown(self):
        ...
        with (
            ...
            patch("sensors.bluetooth_listener.timezone.now"),
        ):
            self.listener.detection_callback(device, advertisement_data)
...
class TestUpdateCacheWithBufferedData:
    ...
    def test_pings_watchdog(self):
        with (
            patch("sensors.bluetooth_listener.get_sensors_data_in_cache", return_value={}),
            patch("sensors.bluetooth_listener.notify_watchdog") as mock_notify,
            patch("sensors.bluetooth_listener.cache.set"),
        ):
    ...
    def test_new_sensor_has_no_previous_measurements(self):
        ...
        with (
            patch("sensors.bluetooth_listener.get_sensors_data_in_cache", return_value={}),
            patch("sensors.bluetooth_listener.notify_watchdog"),
            patch("sensors.bluetooth_listener.cache.set") as mock_cache_set,
        ):
            self.listener.update_cache_with_buffered_data()
    ...
    def test_previously_seen_sensor_keeps_its_last_measurements_as_previous(self):
        ...
        with (
            patch(
                "sensors.bluetooth_listener.get_sensors_data_in_cache",
