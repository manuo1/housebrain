...
def test_get_hc_hp_ref_day_finds_most_recent_complete_day():
    ...
    DailyIndexes.objects.create(date=date(2025, 5, 1), tarif_periods=older_complete)
    DailyIndexes.objects.create(date=date(2025, 5, 20), tarif_periods=newer_complete)
...
def test_get_hc_hp_ref_day_skips_incomplete_day():
    ...
    DailyIndexes.objects.create(date=date(2025, 5, 25), tarif_periods=incomplete)
    DailyIndexes.objects.create(date=date(2025, 5, 20), tarif_periods=complete)
...
def test_get_hc_hp_ref_day_ignores_day_outside_search_window():
    ...
    DailyIndexes.objects.create(
        date=date(2025, 6, 1) - timedelta(days=400), tarif_periods=too_old
    )
...
def test_get_tempo_ref_day_filters_by_color():
    ...
    DailyIndexes.objects.create(date=date(2025, 1, 10), tarif_periods=blue_day)
    DailyIndexes.objects.create(date=date(2025, 1, 15), tarif_periods=red_day)
...
def test_fill_missing_tarif_periods_hc_hp_reuses_ref_day():
    ...
    DailyIndexes.objects.create(date=date(2025, 5, 20), tarif_periods=ref_day)
    tarif_periods = make_tarif_periods(**{"12:00": TarifPeriods.HC})
...
def test_fill_missing_tarif_periods_tempo_reuses_same_color_ref_day():
    ...
    DailyIndexes.objects.create(date=date(2025, 1, 10), tarif_periods=blue_ref_day)
    DailyIndexes.objects.create(date=date(2025, 1, 15), tarif_periods=red_ref_day)
    tarif_periods = make_tarif_periods(**{"12:00": TarifPeriods.HPJR})
